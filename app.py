...
def effective_balance(user_email: str):
  ...
  - אם המשתמש לא קיים → נוצרת רשומת fallback חדשה (used_credits=0, limit=FALLBACK_LIMIT_DEFAULT).
  - אם יש טוקן מוצפן אישי → נבדקת היתרה האמיתית ב-RunPod (account API).
  ...
  try:
      ...
      if not row:
          ...
          supabase.table("accounts").insert(payload).execute()
          need_token = False  # יש לו fallback להתחלה
